...
export default function Details() {
  ...
  return (
    <div>
      <div>
        <div
          className=" flex items-end bg-gray-800 text-white h-[90vh]"
          style={{
          ...
          <div className="w-full h-full flex flex-col md:flex-row justify-end md:items-end p-5 sm:p-10 gap-5 bg-m_black/80">
            ...
            <div className=" flex flex-col sm:flex-row items-end gap-4 w-full sm:w-[80%] justify-between">
              <div className="w-full sm:w-[55%]">
                ...
                <p className=" flex flex-wrap items-center gap-1 text-sm mb-3 mt-1 italic">
                  {`${formatDate(movieDetails.release_date)}`} <TbPointFilled />{" "}
                  {genres.join(", ")} <TbPointFilled />{" "}
                ...
                <div>
                  <Trailer videoKey={movieDetails?.videos?.results?.[0]?.key} />
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <p className=" text-sm">
                      {movieDetails.original_language
                        ? getL(movieDetails.original_language)
                        : ""}
                    </p>
                  </div>
                </div>
                <div>
                  <div>
                    ...
                    <p className=" text-sm mb-2">
                      {formatMoney.format(movieDetails.budget)}
                    </p>
                  </div>
                  <div>
                    <h4 className="text-sm sm:text-base font-bold">Revenue:</h4>
                    <p className=" text-sm">
                      {formatMoney.format(movieDetails.revenue)}
                    </p>
                  </div>
                </div>
                <div>
                  ...
                  <div className=" flex gap-4 mt-3">
                    <a
                      className=" text-m_white hover:text-m_gold"
        ...
        <div>
          <div className=" p-8" style={{ overflowX: "hidden" }}>
            ...
            <div className=" relative flex items-center group">
              ...
              <div
                id={castSliderId}
                className=" w-full h-full overflow-x-scroll whitespace-nowrap scroll-smooth scrollbar-hide relative mx-10"
              >
